...
| Один тенант, микросайта мало | mega-hub market, шаблон `tour_operator` / `guide` |
| Несколько тенантов вместе | mega-hub market, шаблон `visit_center` |
...
Всё это работает не на бумаге: `apps/market` — запускаемое приложение на этом же рендерере и этих же таблицах. Заявка, одобрение, счёт, оплата, появление карточки, правка блоков, статистика показов — реальные записи в Postgres. Оба скина тоже живые страницы, а не картинки: `visit-karakol` — `destination` на нескольких тенантах, `karakol-trails` — `operator` на одном. Как запустить и что показывать — [../sites/KARAKOL.md](../sites/KARAKOL.md).
