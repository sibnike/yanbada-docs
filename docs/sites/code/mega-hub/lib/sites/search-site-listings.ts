...
import { listingMatchesSiteScope, type SiteCompany, type SiteListing, type SiteRow } from '@/types/site'
...
export async function searchSiteListings(
  ...
  const tenantIds = Array.from(new Set(rawListings.map((r) => String(r.tenant_id))))
  ...
  matched.sort((a, b) => Number(b.featured) - Number(a.featured))
  const listings = matched.slice(0, limit)

  const usedTenantIds = new Set(listings.map((l) => l.tenant_id))
